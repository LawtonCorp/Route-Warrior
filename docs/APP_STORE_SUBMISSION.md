...
| 0.4 | **Done (2026-09-23)**, with one part deliberately parked. Key restricted to **iOS apps** with both bundle ids; Routes API `computeRoutes` capped at **1,000/day** with an email alert at 80%; the unused `computeRouteMatrix` daily quota lowered from Unlimited to its minimum; a billing budget set. Confirmed on the phone afterwards: Google's plan still appears. **API restrictions is not set** — the key page in the console did not offer the section, and it was parked rather than fought: the quota caps bound the damage a leaked key could do, which is what API restrictions would otherwise have limited. | `docs/HANDOFF.md` session log 2026-09-16 → 23. **After D-085 (2026-09-25)** the key's iOS-apps restriction must list the new bundle ids, `com.lawtoncorp.routerebel` and `com.lawtoncorp.routerebel.widgets`; until it does, every Google plan and the Google map fail with a 403. |
...
| Bundle ID | `com.lawtoncorp.routerebel` — appears in the menu once a device build has registered it (`./scripts/device-build.sh` does that through `-allowProvisioningUpdates`). If it is missing, register it at developer.apple.com → Identifiers with the App Groups and iCloud capabilities ticked (no Push: the app declares no push entitlement). |
...
| Product ID | `com.lawtoncorp.routerebel.pro.monthly` | `com.lawtoncorp.routerebel.pro.annual` |
...
2. Lock the phone, drive a few minutes (as a passenger): show the phone
   locked, then unlock and show the recorder row "Rec" on the Route tab.
